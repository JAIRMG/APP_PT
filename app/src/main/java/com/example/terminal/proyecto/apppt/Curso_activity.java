...
public class Curso_activity extends AppCompatActivity implements sendData {
...
*/
    ...
    public void countRepeated(){
        ...
        int[] count = new int[3];
        int[] arrayRandom2 = new int[10];
        arrayRandom2 = arrayRandom;
        Arrays.sort(arrayRandom2);

        for(int i = 0; i < arrayRandom2.length; i++) {
            if(arrayRandom2[i] == 1){
                abierta++;
            } else if (arrayRandom2[i] == 2){
                multiple++;
            } else vf++;
        }
